...
include 'includes/dbconnect.php';
	...
	if(isset($_POST['submit'])){
		...
		$ch_sql = "SELECT * FROM accounts WHERE accemail = '$accemail'";
			$chc_sql = mysqli_query($con, $ch_sql);
			$temp = mysqli_affected_rows($con);
			if(!$temp){
		
				$ins_sql = "INSERT INTO accounts(accname, accemail, accpassword, accdate) VALUES ('".$accname."', '".$accemail."', '".$accpassword."', '".$accdate."')";
				$run_sql = mysqli_query($con,$ins_sql);

				$temp = mysqli_affected_rows($con);
				if($temp>0){

					$in_sql = "INSERT INTO users(name, email, password) VALUES ('".$accname."', '".$accemail."', '".$accpassword."')";
