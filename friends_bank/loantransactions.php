...
if(isset($_POST['submit'])){
		...
		$payment_name = $_POST['accname'];
		$payment_no = $_POST['accno'];
		$amount = $_POST['paymentamount'];
		if($payee_name==$name){	
			$ins_sql = "SELECT * FROM loan_payee WHERE accname ='$payment_name'";
...
<div class="container">
	...
	<form class="form-horizontal" action="loanpayment.php" method="post" role="form">
				...
				<div class="form-group">
					<label for="number" class="col-sm-3 control-label">Payment Amount *</label>
						<div class="col-sm-8">
							<input type="text" name="paymentamount" class="form-control" placeholder="Enter the amount" id="loanamount" required>
						</div>
				</div>
				<div class="form-group">
